...
class userService {
  static async register(userData) {
    try {
      ...
      const emailAlreadyExist = await userDao.findByEmail(userData.email);
      if (emailAlreadyExist) throw new Error("Email already exist");

      // if user does not exist, create the user
      const createUser = await userDao.create({
  ...
  static async editUser(userId, userData) {
    try {
      ...
      const emailAlreadyExist = await userDao.findByEmail(user.getEmail()); // check if the email exist
